...
RequestList* reqList;
bool run = TRUE;
static const char* loggerModuleName = "agent++.agent";
...
void init(Mib& mib, const NS_SNMP OctetStr& engineID)
{
    ...
    mib.add(new ucd_snmp_mib());
	mib.add(new host_resources_mib());
	mib.add(new ucd_diskio_mib());
	// mib.add(new if_mib());

	// MibConfigBER ber_format;
	// mib.add_config_format(1, &ber_format);	
	// bool l = mib.load(1, "../mib/UCD-SNMP-MIB.mib.txt");
	// if(l == false){
...
int main (int argc, char* argv[])
{
	...
	// bind localhost only -> agent will not be reachable from outside
